...
from trumpet.models.sitecontent import SiteAppResource
from trumpet.models.sitecontent import UserAppResource
...
class SiteAppManager(object):
    def __init__(self, session):
        self.session = session
        self.dbmodel = SiteAppResource
    ...
    def get_resource(self, name):
        r = self.query().filter_by(name=name).one()
        return r.content

    def add_resource(self, name, content, user_id):
        with transaction.manager:
            r = SiteAppResource()
            r.name = name
            r.content = content
            r.created_by_id = user_id
            r.modified_by_id = user_id
            now = datetime.now()
            r.created = now
            r.modified = now
            self.session.add(r)
        return self.session.merge(r)

    def update_resource(self, id, user_id,
                        name=None,
                        content=None,):
        with transaction.manager:
            r = self.query().get(id)
